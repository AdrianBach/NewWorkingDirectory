...
void makeDietsTable(bool display) // this table allows for each member of the system to eat and be eaten by another
{
    ...
    */
    ...
    dietsTable[3][4] = 2;   // predator1 feeds on prey2
        ...
        */
        ...
        while (ind < currentPopulationSize) // iterate through individuals
        {
            ...
            while (i < availablePreys.size() && dailyCons < dailyPredMaxConsumption && predCons < predMaxConsumption)
            {
                ...
                int dens = preysDensity[preyType];
                int convRate = conversionRates[preyType];
                float catchProb = catchProbas[preyType];
                int densColumn = dietLandscapeIndexes[preyType];
                int catchColumn = densColumn + preyTypesNb;
        ...
        */
        ...
        if (timeStep > 0)
        {
            ...
            if (timeStep % freqSurv == 0)
            {
                ...
                {    
                    if (timeStep >= preyIntro[i])
                        preys[i]->survivalTrial(false, false);
                }
                // prey1.getInfo();
                // prey2.getInfo();
                ...
                /* predators */
                if (timeStep >= predIntro[0])
                    pred1->survivalTrial(false, false);
