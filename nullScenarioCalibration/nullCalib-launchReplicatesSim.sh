...
prd_cvrt_pry2_1=$prd_cvrt_pry1_1  # argv[31] predator 1 prey1 resources/catch
...
# sim values arrays
prey1init=(0 100)
predSurvArray=(50 100 300 1000)
predOffsArray=(0.1 0.5 1 3)
...
echo "predOffsArray is ${predOffsArray[*]}"
echo "predOffsArray size is ${#predOffsArray[@]}"
...
do 
    ...
    do
        ...
        do
            prd_offs_1=${predOffsArray[$m]}  

            # loop over predSurvArray
            for ((k=0 ; k<${#predSurvArray[@]} ; k++))
            do
                prd_surv_1=${predSurvArray[$k]}
                prd_repr_1=$prd_surv_1
                divide=$((3*$prd_surv_1)); by=$freq_surv; prd_cons_1=`echo "scale=0; ($divide+$by-1)/$by" | bc`;

                # name the simulation with only the variables of interest and their value
                sim_name="nullCalib-py1Init$pry_init_1-prdSurv$prd_surv_1-prdOffs$prd_offs_1-prdCtPr$prd_ctch_pry1_1" # argv[1]
