...
mergeResults <- function(path, keyword = c("Results", "Snapshot"), pattern) {
  ...
  for (i in 1:length(content)) {
    ...
    for (j in 1:length(simFol)) {
      ...
      # get csv files
      results <- list.files(resFol, pattern = ".csv")
      ...
      for (m in 1:length(results)) {
        ...
        # read results
        res <- read.csv(file.path(resFol, results[m]))
...
localSAresults <- function(path, keyword = c("Results", "Snapshot"), pattern, burnTime, initTime, maxTime) {
  ...
  headers <- c("predSpecific", "predOportunistic", "convRateRatio", "catchProbaRatio", "preyOffspRatio", "maxConsRatio", "replicatesNb",
               ...
               "r2", "r2sd", "r2R2",
               "alpha22invK", "alpha22invKsd",
               "alpha22lm", "alpha22lmSD", "alpha22lmR2")
  ...
  for (i in 1:length(content)) {
    ...
    for (j in 1:length(simFol)) {
      ...
      # HERE GO r2 and alpha22 estimations
      
      # browse stats folder and read stats
      merged <- list.files(paste(resFol, statsFol, sep = "/"))
      merged <- grep(pattern = c("merged"), x = merged, value = T)
      merged <- grep(pattern = c(".csv"), x = merged, value = T)
      
      # import data set
      merged <- read.csv(file = file.path(paste(resFol, statsFol, sep = "/"), merged), header = T, stringsAsFactors = T, row.names = F)
      
      # subset to the first time steps when they invade from rare
      dataset <- subset(merged, merged$timeStep < initTime)
      
      # fit model
      mod <- lm(formula = log(dataset$prey2PopulationSize) ~ dataset$timeStep)
      out <- summary(mod)
      
      # update new line
      newLine <- c(newLine,
                   out$coefficients[2,1], out$coefficients[2,2], out$adj.r.squared)
      ...
      dataset <- subset(merged, merged$timeStep >= burnTime)
      ...
      newLine <- c(newLine,
                   1/mean(Nt), 1/sd(Nt),
                   -out$coefficients[2,1]/out$coefficients[1,1], NA, out$adj.r.squared)
      
      # eventually plot the mathematics formula over the raw data and save in the allStatsAndPlots subfolder 
      
      # rbind the new line to tab
    ...
    newFolderDir <- paste(folder, "allStatsAndPlots", "intraSpecComp-localSAfiles", sep = "/")
    dir.create(path = newFolderDir)
...
Path = "C:/Users/adb3/Desktop/PhD/GitKraken/Chapter2model/intraSpecComp/"
Pattern = "intraSpecComp-pred"
Keyword = "Results"
...
######### inter-specific competition parameter estimation ########

#### cbind the new columns in  ####
