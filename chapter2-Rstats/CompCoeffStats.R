...
alpha.intra.est <- function(path, keyword = c("Results", "Snapshot"), tburn) {
  ...
  for (i in 1:length(content)) {
    ...
    for (j in 1:length(simFol)) {
      ...
      # exclude burning ts
      subburn <- subset(stats, stats$timeStep >= tburn)
      
      # for each replicate, estimate alpha ii with the 2 methods
      repNb <- max(stats$replicate)
      
      # vector to store estimations
      alphaiiLM <- rep(0, repNb)
      riLM <- rep(0, repNb)
      alphaiiK <- rep(0, repNb)
      
      for (k in 1:repNb) {
        subrep <- subset(subburn, subburn$replicate == as.factor(k))
        
        # small transformation of the data to be able to fit Nt to deltaN
        Nt <- subrep$prey2PopulationSize[-length(subrep$prey2PopulationSize)]
        deltaNt <- subrep$prey2growthRate[-1] 
        
        # estimate with lm 
        linearMethod <- lm(formula = deltaNt/Nt ~ Nt)
        
        riLM[k] <- linearMethod$coefficients[1]
        alphaiiLM[k] <- -linearMethod$coefficients[2]/linearMethod$coefficients[1]
        
        # double check
        subrepNoburn <- subset(stats, stats$replicate == as.factor(k))
        NtNoburn <- subrepNoburn$prey2PopulationSize[-length(subrepNoburn$prey2PopulationSize)]
        deltaNtNoburn <- subrepNoburn$prey2growthRate[-1] 
        linearMethodNoburn <- lm(formula = deltaNtNoburn/NtNoburn ~ NtNoburn)
        
        par(mfrow = c(1,2))
        
        plot(y = deltaNt/Nt, x = Nt)
        abline(a = linearMethod$coefficients[1], b = linearMethod$coefficients[2], col = "red")
        
        plot(y = deltaNtNoburn/NtNoburn, x = NtNoburn)
        abline(a = linearMethodNoburn$coefficients[1], b = linearMethodNoburn$coefficients[2], col = "red")
...
missingSet <- read.csv(file = "/home/adrian/Documents/GitKraken/Chapter2model/localSA/folder-localSA-ConvRate/allStatsAndPlots/localSAfiles/stats-folder-localSA-ConvRate.csv")
...
# convRate
filePath = "/home/adrian/Documents/GitKraken/Chapter2model/localSA/folder-localSA-ConvRate/allStatsAndPlots/localSAfiles/stats-folder-localSA-ConvRate.csv"
tab <- read.csv(file = filePath)
tab <- rbind(tab, missingLine)
write.csv(tab, file = filePath, row.names = FALSE)
...
folderPath = "/home/adrian/Documents/GitKraken/Chapter2model/localSA/folder-localSA-ConvRate-over1/allStatsAndPlots/localSAfiles/"
filePath = "/home/adrian/Documents/GitKraken/Chapter2model/localSA/folder-localSA-ConvRate-over1/allStatsAndPlots/localSAfiles/stats-folder-localSA-ConvRate-over1.csv"
data <- read.csv(filePath)
...
fig <- ggplot(data, aes(x)) + 
  ...
  geom_line(aes(y = y3), color = y3c, alpha = 0.2, position = position_nudge(x = 0.1)) +
  # geom_hline(yintercept = 0, color = "darkgreen", linetype = "dashed") +
  geom_point(aes(y = y1), size = 3, shape = 21, fill = "white", color = y1c, position = position_nudge(x = -0.1)) +
  geom_point(aes(y = y2), size = 3, shape = 22, fill = "white", color = y2c) +
  geom_point(aes(y = y3), size = 3, shape = 24, fill = "white", color = y3c, position = position_nudge(x = 0.1)) +
...
fig <- ggplot(data, aes(x)) + 
  ...
  geom_pointrange(aes(y = y1, ymin = y1min, ymax = y1max), shape = 21, fill = "white", size = 0.5, col = y1c, position = position_nudge(x = -0.075)) +
